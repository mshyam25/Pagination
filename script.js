...
// Getting the four buttons which will be displaying initially
...
let rowsPerPage = 10;
let totalButtons = Math.ceil(data.length / rowsPerPage);
let page = 0;

//Event for First,Previous,Next,End Buttons
    ...
    button.addEventListener('click', () => {
        if (button.innerText == "FIRST") {
            if (page == 1)
                ...
                page = parseFloat(first.innerText);
                first.focus();
                prevButton.blur();
                prevButton = first;
            }
            ...
            if (!(first.innerText == '1')) {
                first.innerText = parseFloat(first.innerText) - 1;
                second.innerText = parseFloat(second.innerText) - 1;
                third.innerText = parseFloat(third.innerText) - 1;
                fourth.innerText = parseFloat(fourth.innerText) - 1;
                // need to call the focusblur function
                page = page - 1;

            } else if (first.innerText == '1' && !(page == 1)) {
                if (!(page == 0))
                    page = page - 1;
                //need to call the focusblur func
                else
                    alert('Select a page to display');
            } else
                alert('You are already at the first page');

        } else if (button.innerText == "NEXT") {
            if (!(fourth.innerText == totalButtons) && !(page == 0) && !(page == totalButtons)) {
                first.innerText = parseFloat(first.innerText) + 1;
                second.innerText = parseFloat(second.innerText) + 1;
                third.innerText = parseFloat(third.innerText) + 1;
                fourth.innerText = parseFloat(fourth.innerText) + 1;
            ...
            } else if (fourth.innerText == totalButtons && !(page == totalButtons)) {
            ...
            if (page == totalButtons)
                ...
                first.innerText = totalButtons - 3;
                second.innerText = totalButtons - 2
                third.innerText = totalButtons - 1;
                fourth.innerText = totalButtons;
                prevButton.blur();
                fourth.focus();
                prevButton = fourth;
            }
        ...
        let val = page;
        table.innerHTML = '';
        for (let i = (val * rowsPerPage - rowsPerPage); i < val * rowsPerPage; i++)
